...
class AccountVoucherType(orm.Model):
    ...
    def _check_range_percentage(self, cr, uid, ids, context=None):
        for record in self.read(cr, uid, ids, ['percentage']):
            if record['percentage'] in range(0, 101):
                return True
        return False
    _constraints = [
        (_check_range_percentage, 'Error: el porcentaje debe estar entre 0 y 100%', ['percentage'])
    ]
